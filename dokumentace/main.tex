...
Třída \texttt{TimelineWidget} se stará o zobrazení průběhu celého aranžmá v čase. Zobrazuje horizontální osu s časovým měřítkem, waveform výsledného mixu a polohu přehrávací hlavy. Uživatel může kliknutím na časovou osu měnit pozici přehrávání, případně nastavovat oblast loopu pomocí dvou značek (A/B). Widget také emituje signály \texttt{seekRequested}, \texttt{loopAChanged}, \texttt{loopBChanged} a \texttt{clearLoopRequested}, na které \texttt{MainWindow} reaguje aktualizací stavu přehrávače a editoru.
...
\texttt{TrackEditorWidget} umožňuje buňky horizontálně a vertikálně posouvat, kopírovat a dělit. Pomocí kontextového menu lze na vybranou buňku aplikovat efekty (gain, fade-in, fade-out a echo), nebo buňku rozdělit na dvě části v zadaném čase. Widget také podporuje přetahování souborů z operačního systému a položek z knihovny skladeb – při dropu vytvoří novou buňku s odpovídajícím waveformem a délkou. Při každé změně aranžmá vysílá signál \texttt{arrangementChanged}, na který hlavní okno reaguje přepočtem celkové délky a přípravou mixdownu.
...
Součástí aplikace \textit{prazmas} je průvodce pro automatické generování practice mixů, který má trenérům ušetřit čas při přípravě tréninkových bloků. Při spuštění generování uživatel zadá několik parametrů: délku jednoho tance, délku pauzy mezi jednotlivými tanci, volbu typu sekvence (standardní \mbox{STT} nebo latinské \mbox{LAT}) a možnost, zda se má hotový mix uložit do knihovny pro pozdější znovupoužití.
...
\subsection*{Uložení projektu a export mixu}

Jakmile je mix hotový, může uživatel projekt uložit, aby se k němu později mohl vrátit. K tomu slouží volby v hlavním menu, které uloží rozložení buněk a nastavení do projektového souboru. Pro použití mimo aplikaci se využije funkce \emph{Export}, která provede mixdown a uloží výsledný soubor jako WAV; podle nastavení může být následně překódován do MP3 nebo FLAC. Exportovaný soubor lze přenést na jiné zařízení nebo použít v běžném přehrávači na tréninku.
...
\bibitem{pyqt6}
Qt for Python Documentation, ``PyQt6 Reference Guide'',
...
Riverbank Computing Limited, ``Qt Designer Manual'',
